import { Component, OnInit, Input, Output, EventEmitter, ViewChild, ViewChildren, ElementRef, SimpleChanges } from '@angular/core';
...
import { AmisControlGroupRowComponent } from '../../amis-control-group/amis-control-group-row/amis-control-group-row.component';
...
@Component({
  selector: 'amis-amis-form-field',
  templateUrl: './amis-form-field.component.html',
  styleUrls: ['./amis-form-field.component.scss']
})
export class AmisFormFieldComponent extends BaseComponent implements OnInit {
 ...
 */
  ...
  @ViewChild("groupRow")
  groupRow: AmisControlGroupRowComponent;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (this.checkInputChange(changes, "groupConfig")) {
      if (this.groupConfig) {
        this.setDataForm();
  ...
  setDataForm() {
    ...
    this.setDataGroupConfig(this.groupConfig);
  ...
  setFormulaValueDetail(e) {
    ...
    if (index != -1) {
      ...
      this.listGroupFieldConfig[index].Value = executeFormula(normalizeParamsObj.text, normalizeParams);
    }
  }
  ...
  validatedForm() {
    ...
    if (!this.groupRow?.isValidField()) {
      validateList.push(this.groupRow);
    }
  ...
  beforeUpateField(obj) {
    this.groupRow?.beforeEditValue(obj);
  }
  ...
  updateFieldData(param: GroupFieldConfig, field: BaseControl) {
    const me = this;
    this.amisDataService.updateField(param.SubsystemCode, param).subscribe(res => {
      if (res?.Success) {
  ...
  */
  onClickNo(e) {
    this.onClickNoEvent.emit([this.groupConfig]);
  }
  ...
  onClickYes(e) {
    if (this._formMode == FormMode.View && this.currentItem && this.currentField) {
      ...
      if (e) {
        this.onClickYesEvent.emit([this.groupConfig]);
      }
    }
  }
