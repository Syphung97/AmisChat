...
export class PdfViewerComponent
  ...
  @Output() pageSizeChange: EventEmitter<number> = new EventEmitter<number>(true);
  @Input()
  ...
  set pageSize(_pageSize) {
    if (this._pdf) {
      this._pageSize = this._pdf.numPages;
    }
    this.pageSizeChange.emit(_pageSize);
  }
  ...
  private loadPDF() {
    ...
    (<PDFPromise<PDFDocumentProxy>>this.loadingTask.promise).then(
      (pdf: PDFDocumentProxy) => {
        ...
        this.resetPdfDocument();
        this.pageSizeChange.emit(this._pdf.numPages);
        this.update();
      },
      (error: any) => {
